...
class ConversationalAgentsHandler():
    ...
    def __init__(self, agent_logic_actions:BaseConversationalAgentActionsCollection, post_processing_pipeline: PostProcessingPipeline):
    ...
    def get_by_user_id(self, user_id: str, decision_agent:BaseDecisionAgent):
        if user_id in self.conversational_agents:
            ...
            if self.conversational_agents_type == 'simple':                
                ...
                new_ca = ConversationalAgentSimple(user_id=user_id, prompts=prompts, decision_agent=decision_agent, agent_logic=agent_logic, guiding_instructions=guiding_instruction, post_processing_pipeline=self.post_precessing_pipeline)
                self.conversational_agents[user_id] = new_ca

            elif self.conversational_agents_type == 'rag':
                prompts = prompt_loader.get_all_prompts()               
                new_ca = ConversationalAgentRAG(user_id=user_id, prompts=prompts, decision_agent=decision_agent, agent_logic=agent_logic, guiding_instructions=guiding_instruction, post_processing_pipeline=self.post_precessing_pipeline)
                self.conversational_agents[user_id] = new_ca  
